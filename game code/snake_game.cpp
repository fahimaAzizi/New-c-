...
void Draw() {
    system("cls"); // Clears the console screen (use "clear" on Linux/Unix systems)
...
void Logic() {
    ...
    if (x == fruitX && y == fruitY) {
        ...
        fruitX = rand() % width;
        fruitY = rand() % height;
        nTail++; // Increase the snake's tail
    }
...
int main() {
    ...
    while (!gameOver) {
        ...
        _sleep(100); // Sleep for 100 milliseconds (use usleep on Unix-based systems)
    }
    return 0;
}
